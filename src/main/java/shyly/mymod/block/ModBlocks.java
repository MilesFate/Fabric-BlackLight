...
import shyly.mymod.world.tree.RedMapleSaplingGenerator;
...
public class ModBlocks {
    ...
    private static Block registerBlock(String name, Block block, ItemGroup group){
        ...
        return Registry.register(Registries.BLOCK, new Identifier(myMod.MOD_ID,name), block);
    }
    private static Item registerBlockItem(String name, Block block, ItemGroup group){
        Item item = Registry.register(ITEM, new Identifier(myMod.MOD_ID, name), new BlockItem(block, new FabricItemSettings()));
        ItemGroupEvents.modifyEntriesEvent(group).register(entries -> entries.add(item));
        return item;
    }

    public static void registerModBlocks(){
        myMod.LOGGER.info("Registering ModBlocks for " + myMod.MOD_ID);
    }
}
